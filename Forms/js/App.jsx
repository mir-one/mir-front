import MirModel from "./MirModel";
import {MirForm} from "./components/MirForm";
...
const MirFormStyled = withStyles(styles)(MirForm);
...
window.ttmForm = function (div, form_address, sess_hash_in, post, get, input) {
    ...
    if (sess_hash) {
        model = new MirModel(form_address, sess_hash);
    } else {
        model = new MirModel(form_address);
    }
    ...
    model.load(post, get, input).then((json) => {
        if (json.error) {
            ...
            ReactDom.render(<MirFormStyled data={json} container={div} model={model}/>, div);
        }
    }).catch(renderError);
    ReactDom.render(
        <Trobber/>

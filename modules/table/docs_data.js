$(function () {
    ...
    const addDocsClick = function () {
        ...
        $.post('https://docs.mir.one/index_'+type+'.json', function (json) {
            if (json && json.length){
                json.forEach(function(row){
                    selectDiv.append('<div style="'+(row[2] || "")+'"><i class="fa fa-external-link"></i> <a href="http://docs.mir.one'+row[1]+'" target="mir-docs">'+row[0]+'</a></div>');
                });
            }
        });

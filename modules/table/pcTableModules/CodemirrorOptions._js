(function () {
    ...
    const MirTab = function (cm, alt, shift) {
        ...
        return {
            ...
            token: function (stream, state) {
                ...
                if (!state.inFunction && /[a-zA-Z]/.test(stream.peek())) {
                    ...
                    let func = MIRjsFuncs[streamString().trim().toLowerCase()];
    ...
    function autoCloseS(cm) {
        if (cm.getOption("disableInput") || cm.getOption("mode") !== 'mir') return CodeMirror.Pass;
        ...
        for (let i = 0; i < ranges.length; i++) {
            ...
            if ((tok.state && tok.state.inFuncName) || tok.type == 'function' || tok.type == 'error') {
                ...
                if (info) {
                    ...
                    if (openHints) {
                        CodeMirror.showHint(cm, CodeMirror.hint.mirVars, {});
                    }
                }
            } else {
    ...
    CodeMirror.registerHelper("hint", "mirVars", function javascriptHint(editor, options) {
        return scriptHint(editor, mirKeywords,
            function (e, cur) {
    ...
    let mirAutoCloses = function (cm) {
        ...
        cm.on('keydown', function (cm, event) {
            if ((event.keyCode || event.which).toString() === '9') {
                if (MirTab(cm, event.altKey, event.shiftKey)) {
    ...
    CodeMirror.defineOption("autoCloseFunctions", false, function (cm) {
        if (cm.getOption('mode') === 'mir') mirAutoCloses(cm);
